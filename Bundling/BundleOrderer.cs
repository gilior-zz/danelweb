...
namespace Danel.WebApp.Bundling
{
    /// <summary>
    /// Knows how to order bundle based on JavaScript script references embedded inside the script
    /// </summary>
    public class BundleOrderer : IBundleOrderer
    {
        ...
        public IEnumerable<BundleFile> OrderFiles(BundleContext context, IEnumerable<BundleFile> filesIn)
        {
            ...
            foreach (BundleFile bundleFile in files)
            {
                ...
                {
                    //
                    //  In rare cases a bundle might contain a file that was just deleted
                    //  For example, delete a file that is not part of the VisualStudio project
                    //
                    continue;
                }
        ...
        private string[] ParseDepenedencies(BundleContext context, BundleFile bundleFile, FileInfo file)
        {
            ...
            using (StreamReader reader = File.OpenText(context.HttpContext.Server.MapPath(bundleFile.VirtualFile.VirtualPath)))
            {
                ...
                while (true)
                {
                    ...
                    if (!dependencyRelativePath.EndsWith(".d.ts"))
                    {
                        //
                        //  Typescript definition file is not treated as a real dependency
                        //
