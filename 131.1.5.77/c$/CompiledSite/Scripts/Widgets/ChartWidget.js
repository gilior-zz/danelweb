...
(function (Danel) {
    ...
    var ChartWidget = (function () {
        function ChartWidget(scope, element, attrs, ddo, parameterService) {
            var me = this;
            me.scope = scope;
            me.element = element;
            me.attrs = attrs;
            ...
            seriesArr.push();
            parameterService.GetDanelParameter(Danel.WebParameter.ColorsPlate).then(function (res) {
                var definitions = res.ParameterItems[0].Value.split('^');
            ...
            if (me.attrs.danelChartAreaMargin) {
                var str = me.attrs.danelChartAreaMargin;
                var parts = str.split(",");
                if (parts.length == 1) {
        ...
        ChartWidget.prototype.setChartOptions = function (options) {
            var me = this;
            me.kendoChart.setOptions(options);
        };
        ChartWidget.prototype.validateAttrs = function (attrs) {
            var me = this;
