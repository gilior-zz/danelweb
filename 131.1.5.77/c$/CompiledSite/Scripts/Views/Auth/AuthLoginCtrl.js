...
(function (Danel) {
    var AuthLoginCtrl = (function (_super) {
        ...
        function AuthLoginCtrl($scope, $state, $timeout) {
            _super.call(this, "AuthLoginCtrl", $scope);
            var me = this;
            ...
            me.watchData();
            var req = { webParameter: Danel.WebParameter.WebSiteSendingType + Danel.WebParameter.WebSiteRegistrationType + Danel.WebParameter.WebSiteUrl + Danel.WebParameter.CompanyDisplayName };
            me.httpService.POST('/api/Parameters/GetParameter', req).then(function (res) {
                me.CompanyDisplayName = res.ParameterItems[0].Value;
                me.WebSiteRegistrationType = +res.ParameterItems[1].Value;
                me.WebSiteSendingType = +res.ParameterItems[2].Value;
                ...
                me.webSiteUrl = res.ParameterItems[3].Value;
        ...
        AuthLoginCtrl.prototype.openNewRegisterWindow = function () {
            ...
            me.applyChanges(function () {
                $(".k-window-title").html("הצרפות לשירות");
            });
            me.enableAllButtons();
            var win = me.datePicker.data("kendoDatePicker");
            win.value(null);
        ...
        AuthLoginCtrl.prototype.openForgotUserNameWindow = function () {
            ...
            me.enableAllButtons();
            if (!me.forgotIDMsg) {
                var req = { webParameter: Danel.WebParameter.ForgotIDMsg };
                me.httpService.POST('/api/Parameters/GetParameter', req).then(function (res) {
                    me.applyChanges(function () { me.forgotIDMsg = res.ParameterItems[0].Value; });
                }).fail(function (err) {
                    me.applyChanges(function () { me.forgotIDMsg = "יש ליצור קשר עם בית ההשקעות"; });
                });
            }
        };
                ...
                me.applyChanges(function () {
                    switch (error) {
                        case Danel.ErrorCode.AlreadyRegisteredIdentityNumber:
                            me.$scope.saveResult = "לתעודת זהות זו כבר מקושר חשבון אינטרנט. נא לבצע תהליך שחזור סיסמא או ליצור קשר עם " + me.CompanyDisplayName;
                            break;
                        ...
                        case Danel.ErrorCode.AlreadyRegisteredPhone:
                            me.$scope.saveResult = "אירעה תקלה יש ליצור קשר עם " + me.CompanyDisplayName;
                            ;
                            break;
